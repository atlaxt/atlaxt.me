...
function setCopyRef(el: Element | ComponentPublicInstance | null, index: number) {
...
function debouncedUpdateWidths() {
  if (resizeTimeout) {
...
const scrollTransforms = computed(() => {
  return props.texts.map((_, index) => {
    ...
    if (singleWidth === undefined || singleWidth === 0)
      return '0px'
    return `${wrap(-singleWidth, 0, baseX.value[index] || 0)}px`
  })
})

function updateSmoothVelocity() {
...
onMounted(async () => {
  ...
  baseX.value = Array.from({ length: props.texts.length }).fill(0) as any
  copyWidths.value = Array.from({ length: props.texts.length }).fill(0) as any
  calculatedCopies.value = Array.from({ length: props.texts.length }).fill(15) as any
  directionFactors.value = Array.from({ length: props.texts.length }).fill(1) as any
...
<template>
  <section>
    ...
    >
      <div
        ref="scrollerRef"
        :class="`${scrollerClassName} flex whitespace-nowrap text-center font-sans text-4xl font-bold tracking-[-0.02em] drop-shadow md:text-[5rem] md:leading-20`"
